<div class="table-responsive">
    @if (Cart::count() <= 0) <div class="alert alert-warning" role="alert">
...
<table class="table table-striped">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <input id="quantity" type="number" name="quantity" value="{{ $quantity = $product->qty }}"
                    data-url="{{ url("/carts/update", ["id" => $product->rowId ]) }}" data-token="{{ csrf_token() }}">
            </td>
            <td>{{ number_format($price * $quantity, 2) }}</td>
            <td>
                <a id="cartRemove" href="{{ url("/carts", ["id" => $product->rowId ]) }}"
                    data-token="{{ csrf_token() }}"><i class="fa fa-times" style="color: brown"></i></a>
            </td>
        </tr>
        @endforeach
        <hr />
